...
\begin{itemize}
  ...
  \item This deck uses the \textbf{tightened} ladder (not the old $n{=}60$ identical-seed run).
\end{itemize}
...
\begin{block}{\small Operating points}
...
%------------------------------------------------------------------
\begin{frame}{Hardening: seeds actually differ now}
\begin{itemize}
  \item Old $n{=}60$ ``two seeds'' matched cell-for-cell (greedy + same first-$n$).
  \item Tightened: \texttt{--sample\_items} draws different $n{=}100$ subsets per seed.
  \item GSM8K $B{=}16$: $0.79$ (s42) vs $0.68$ (s123); both hit $0.87$ at $B{=}64$.
  \item MedQA pool size $=100$ $\Rightarrow$ seeds identical (full set).
\end{itemize}
\begin{alertblock}{Claim strength}
Operating points hold across two real item-subsets + within-run bootstrap CIs.
\end{alertblock}
\end{frame}
...
\begin{itemize}\small
  ...
  \item ASC remains the orthogonal length knob at chosen operating points.
\end{itemize}
\end{frame}
...
\vspace{0.8em}
\begin{center}
\large\textbf{The scaffold compresses --- but not uniformly.}
\end{center}
\end{frame}
